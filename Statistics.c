...
int64_t InformStatistics(Statistics** stats,int first_array, int first_col,uint64_t first_num_cols, int sec_array, int sec_col,uint64_t sec_num_cols, int type){
	...
	if(sec_array==-1){				// Filter
		...
		if(type==0){				// equal
			...
			if(stats[first_array][first_col].Ua - stats[first_array][first_col].Ia +1 <=N){
				...
				if(stats[first_array][first_col].check[(sec_col-stats[first_array][first_col].Ia)%N]==true){
					stats[first_array][first_col].Fa= stats[first_array][first_col].Fa/ stats[first_array][first_col].Da;
					stats[first_array][first_col].Da=1;
				}
				else{
		...
		return stats[first_array][first_col].Fa;
		...
		if(first_array == sec_array){								// Self Join
			...
			for(i=0; i< sec_num_cols ; i++)				// for the rest columns
				if(i!=sec_col){
					...
					stats[sec_array][i].Fa=  stats[first_array][first_col].Fa; 
...
//		return 0;
	...
	//	InformStatistics(stats, A->firstArray,A->firstCol,Table[array[A->firstArray]].columns, A->secondArray, A->secondCol,Table[array[A->secondArray]].columns,A->commandType);
		...
		if(InformStatistics(stats, A->firstArray,A->firstCol,Table[array[A->firstArray]].columns, A->secondArray, A->secondCol,Table[array[A->secondArray]].columns,A->commandType) == 0)
			return -1;
		while(B!=NULL){
...
int newSort(COMMANDLIST *commandListHead,int* array, Initial_Table* Table,Statistics** initStats, Statistics** stats){
	...
	while( cur!=NULL && cur->secondArray == -1 ){
		if( InformStatistics(stats, cur->firstArray,cur->firstCol,Table[array[cur->firstArray]].columns, -1, cur->secondCol,0,cur->commandType) == 0)
			return -1;
		num_filters++;
		cur = cur->next;
	}
	...
	while(cur != NULL && cur->firstArray == cur->secondArray){	
		if( InformStatistics(stats, cur->firstArray,cur->firstCol,Table[array[cur->firstArray]].columns, cur->secondArray, cur->secondCol,Table[array[cur->secondArray]].columns,cur->commandType) == 0)
			return -1;
		cur = cur->next;
	}
