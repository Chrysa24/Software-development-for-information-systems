...
void commandListSort(COMMANDLIST *commandListHead){
	...
	while(temp->next != NULL){
		...
		/*while finding nodes that are compatible with the temp one, search for next */
		while( 
			((cur->firstArray == temp->firstArray) && (cur->firstCol == temp->firstCol))
			|| 
			((cur->secondArray == temp->secondArray) && (cur->secondCol == temp->secondCol))
			|| 
			((cur->firstArray == temp->secondArray) && (cur->firstCol == temp->secondCol))
			|| 
			((cur->secondArray == temp->firstArray) && (cur->secondCol == temp->firstCol))){
		...
		/*same thing as filter and self-join happens here too, but this time is for when our temp node, has the same array 
		as the search node, this will result in having all the same-array-id nodes close to one another */

		while(search->next != NULL){

			search = search->next;
			if(
			((search->firstArray == temp->firstArray))
			|| 
			((search->secondArray == temp->secondArray))
			|| 
			((search->secondArray == temp->firstArray))
			|| 
			((search->firstArray == temp->secondArray))){
				if(search && cur){
					swapWorkNodes(search, cur);
					cur = cur->next;
				}
		...
		/*		while(search->next != NULL){
			...
			search = search->next;
			if(
			((search->firstArray == temp->firstArray) && (search->firstCol == temp->firstCol))
			|| 
			((search->secondArray == temp->secondArray) && (search->secondCol == temp->secondCol))
			|| 
			((search->secondArray == temp->firstArray) && (search->secondCol == temp->firstCol))
			|| 
			((search->firstArray == temp->secondArray) && (search->firstCol == temp->secondCol))){
				if(search && cur){
					swapWorkNodes(search, cur);
					cur = cur->next;
				}
